...
import ContractDatesBlock from '@/components/ContractDatesBlock'
import {
  ContractDates,
...
//   3. Contract & Schedule Dates (NEW — replaces Key Dates & Durations)
//   4. KPI tiles (4 metrics)
//   5. Schedule Progress chart
//   6. Immediate Attention Areas
//   7. 2 Weeks Lookahead
...
function DashboardContent({ project, version }: { project: Project; version: ScheduleVersion }) {
  ...
  // ============ CONTRACT DATES — read from version (snapshot) ============
  // contractDates lives on the version. If absent, dashboard shows banner to add them.
  const contractDates: ContractDates | undefined = (version as any).contractDates
  ...
  // XER dates — read from analyzer, no defaulting to dataDate
  const xerNTP = a.planStartDate || a.plan_start_date || a.projectStart || a.project_start || a.ntp || a.ntpDate
  const xerProjectedEnd = a.projectedEnd || a.projected_end || a.scd_end_date || a.scdEndDate
                       || a.planEndDate || a.plan_end_date || a.forecastFinish || a.forecast_finish

  // ============ DURATIONS — contract dates are the source of truth ============
  // If contract dates exist, use them. Otherwise fall back to old behavior.
  const originalDuration = contractDates?.originalDuration ?? num(a.originalDuration, 0)
  const revisedDuration = contractDates ? computeRevisedDuration(contractDates) : originalDuration
  const revisedCompletion = contractDates ? computeRevisedCompletion(contractDates) : null

  // Days Behind = XER projected end - Revised Contract Completion (calendar days)
  let daysBehind = 0
  if (contractDates && xerProjectedEnd && revisedCompletion) {
    daysBehind = calendarDaysBetween(revisedCompletion, xerProjectedEnd.slice(0, 10)) - 1
    if (daysBehind < 0) daysBehind = 0
  } else {
    daysBehind = num(a.daysBehind, 0)
  }

  const remainingDuration = num(a.remainingDuration, 0)
  // Duration at Completion = NTP → projected end (calendar days, inclusive)
  let durationAtCompletion = num(a.durationAtCompletion, 0)
  if (contractDates && xerProjectedEnd) {
    durationAtCompletion = calendarDaysBetween(contractDates.ntpDate, xerProjectedEnd.slice(0, 10))
  } else if (durationAtCompletion === 0) {
    durationAtCompletion = originalDuration + daysBehind
  }

  const workComplete = num(a.workComplete ?? a.percentComplete, 0)
  const completedActivities = num(a.completedActivities, 0)
  ...
  const communicationSummary = a.communicationSummary || defaultCommSummary(daysBehind)

  // ============ Schedule Progress chart ============
  // Use contract dates for start and end when available
  const chartStart = contractDates?.ntpDate || xerNTP || dataDate
  const chartContractEnd = revisedCompletion || xerProjectedEnd
  const chartProjectedEnd = xerProjectedEnd || revisedCompletion

  const chartData = useMemo(
    () => buildScheduleProgressData({
      projectStart: chartStart,
      contractEnd: chartContractEnd,
      projectedEnd: chartProjectedEnd,
      dataDate,
      workComplete,
    ...
    [chartStart, chartContractEnd, chartProjectedEnd, dataDate, workComplete, a.plannedVelocity, a.actualByMonth, a.plannedByMonth]
  )
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-y-auto">
      ...
      <div className="p-6 max-w-7xl mx-auto w-full space-y-4">
        ...
        {/* SECTION 2: Contract & Schedule Dates (NEW — replaces Key Dates) */}
        <ContractDatesBlock
          projectId={project.id}
          contractDates={contractDates}
          xerNTP={xerNTP}
          xerProjectedCompletion={xerProjectedEnd}
        />

        {/* Durations summary cells (uses contract truth) */}
        {contractDates && (
          <Card>
            <SectionTitle>Durations</SectionTitle>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <DurationCell label="Original Duration" value={originalDuration} sub="🔒 frozen (calendar days)" />
              <DurationCell label="Revised Duration" value={revisedDuration} sub={`+${contractDates.timeExtensionDays || 0}d extension`} />
              <DurationCell label="Duration at Completion" value={durationAtCompletion} delta={daysBehind} sub="NTP → projected end" />
            </div>
            <div className="mt-2 text-[10px] text-slate-500">
              ⓘ All durations are <strong>calendar days</strong> (inclusive of start and end), per P6 / DCMA convention.
            </div>
          </Card>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <KPITile
            ...
            label="Work Complete"
            value={`${workComplete}%`}
            sub={totalActivities > 0 ? `${completedActivities.toLocaleString()} of ${totalActivities.toLocaleString()} activities` : '—'}
            valueColor="slate"
          />
          <KPITile
            ...
            sub={longLeadAtRisk === 0 ? (longLeadTotal > 0 ? `✓ ${longLeadTotal} long lead total` : '—') : `of ${longLeadTotal} long lead`}
          ...
          <div className="flex items-start justify-between mb-3">
            <div>
              ...
              <div className="text-[11px] text-slate-500 mt-0.5">
                Planned vs Actual completion · Contract end <span className="text-red-600 font-semibold">{fmtDate(chartContractEnd)}</span>
                {' · Projected '}<span className="text-amber-600 font-semibold">{fmtDate(chartProjectedEnd)}{daysBehind > 0 ? ` (+${daysBehind}d)` : ''}</span>
              </div>
            </div>
            <ChartLegend />
          </div>
...
function DurationCell({ label, value, delta, sub }: { label: string; value: number; delta?: number; sub?: string }) {
  return (
    <div>

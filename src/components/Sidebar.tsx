...
// Placeholder counts for the Archive and Deleted sidebar items.
// These match the hardcoded items in the Archive and Deleted pages.
// When real archive/recycle-bin logic ships, derive from projectStore.
const DEMO_ARCHIVE_COUNT = 3
const DEMO_DELETED_COUNT = 3
...
export default function Sidebar({ user }: SidebarProps) {
  ...
  // Admin section - Settings, Archive, Deleted.
  // Placeholder counts; real counts will come from projectStore once
  // status/archive/soft-delete logic ships.
  const adminItems = [
    { href: '/dashboard/settings', icon: '⚙', label: 'Settings', count: null },
    { href: '/dashboard/archive', icon: '📦', label: 'Archive', count: DEMO_ARCHIVE_COUNT },
    { href: '/dashboard/deleted', icon: '🗑️', label: 'Deleted', count: DEMO_DELETED_COUNT },
  ]
    ...
    <aside className="w-64 flex-shrink-0 flex flex-col h-full no-print" style={{ background: '#0d1b2e' }}>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1.5">
        ...
        {searchQuery && filteredProjects.length === 0 && (
          <div className="text-center py-4 text-white/40 text-xs">No matches for "{searchQuery}"</div>
        )}
              ...
              {isEditing ? (
                ...
                <div className="bg-red-500/15 border border-red-500/40 rounded-md p-2 mx-0.5">
                  ...
                  <div className="text-red-300/80 text-[9px] mb-2 truncate" title={p.name}>
                    {p.name} · {p.versions.length} version{p.versions.length !== 1 ? 's' : ''} will be removed
                  </div>
                  <div className="flex gap-1">
                ...
                >
                  ...
                  >
                    ...
                    {p.projectId && (
                      <div className="text-white/40 text-[9px] font-mono mt-0.5 truncate">{p.projectId}</div>
                    )}
                  </div>
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    {isActionMenuOpen && (
                      <div onClick={e => e.stopPropagation()}
                        className="absolute right-0 top-7 z-30 bg-slate-800 border border-white/10 rounded-md shadow-xl py-1 min-w-[130px]">
                        <button onClick={() => startRename(p)}
                          className="w-full text-left px-3 py-1.5 text-[11px] text-white hover:bg-white/10">✏️ Rename</button>
                        <button onClick={() => { setConfirmDeleteProjectId(p.id); setOpenActionMenu(null) }}
                          className="w-full text-left px-3 py-1.5 text-[11px] text-red-400 hover:bg-white/10">🗑️ Delete</button>
                      </div>
                    )}
                  </div>
                </div>
                              ...
                              <div className="max-h-40 overflow-y-auto mb-1">
                                {otherProjects.map(target => (
                                  <button key={target.id}
                                    ...
                                    className="w-full text-left px-2 py-1 hover:bg-white/10 rounded text-[10px] text-white truncate">{target.name}</button>
                                ))}
                              </div>
                            )}
                            <button onClick={() => setMovePickerForVersionId(null)}
                              className="w-full bg-white/10 hover:bg-white/15 text-white text-[10px] py-0.5 rounded">Cancel</button>
                          </div>
                        )
                      }
                      ...
                      return (
                        <div key={v.id}
                          className={clsx(
                            ...
                            isActiveVersion ? 'bg-blue-600/20 border-blue-500' : 'border-transparent hover:bg-white/5'
                          )}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className={clsx('truncate', isActiveVersion ? 'text-white font-semibold' : 'text-white/70')}>{versionLabel}</div>
                            {dateStr && <div className="text-white/30 text-[9px]">{dateStr}</div>}
                          </div>
                          <div className="relative flex-shrink-0">
                            <button
                              onClick={(e) => {
                            ...
                            {isVerActionMenuOpen && (
                              <div onClick={e => e.stopPropagation()}
                                ...
                                {projects.length > 1 && (
                                  <button onClick={() => { setMovePickerForVersionId(v.id); setOpenActionMenu(null) }}
                                    className="w-full text-left px-3 py-1.5 text-[11px] text-white hover:bg-white/10">⇄ Move to…</button>
                                )}
                                {p.versions.length > 1 && (
                                  <button onClick={() => { setConfirmDeleteVersionId(v.id); setOpenActionMenu(null) }}
                                    className="w-full text-left px-3 py-1.5 text-[11px] text-red-400 hover:bg-white/10">🗑️ Delete</button>
                                )}
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={clsx(
                    ...
                    active ? 'bg-blue-600/20 text-white border-blue-500' : 'text-slate-400 border-transparent hover:text-white hover:bg-white/5'
                  )}>
        ...
        {adminItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              <span className="flex-1">{item.label}</span>
              {item.count != null && (
                <span className="bg-white/10 text-white/60 text-[9px] font-bold px-1.5 py-0.5 rounded-full">{item.count}</span>
              )}
            </Link>
          )
        })}
